...
# vae.train()
# 
# vae.save("my_model_1q_or_16q_regressed/", overwrite = True)
# 
# vae = scvi.model.SCVI.load("my_model_1q_or_16q_regressed/", adata=adata_cond)
# 
...
# 
# adata_cond.write_h5ad("output/scanpy/combined_adata_1q_or_16q_regressed.h5ad")

adata_cond = sc.read_h5ad("output/scanpy/combined_adata_1q_or_16q_regressed.h5ad")
...
sc.tl.leiden(filtered_adata_cond, resolution = 0.6)
...
leiden_clusters = filtered_adata_cond.obs['leiden'].unique()

for k in leiden_clusters:
  enrichment[k] = sc.queries.enrich(filtered_adata_cond, k, gprofiler_kwargs = dict(sources=['GO:BP'], no_evidences=False))

test0 = pd.concat(enrichment)
test0 = test0.groupby('name').filter(lambda x: len(x) == 1)

test0.reset_index(inplace=True)

dfs = dict(tuple(test0.groupby('level_0')))

with pd.ExcelWriter('results/enrichment.xlsx') as writer:
  for k in leiden_clusters:
    dfs[k].to_excel(writer, sheet_name=k)


sc.tl.score_genes(filtered_adata_cond, dfs['0'].intersections.iloc[0])

scrb.plot_my_embedding(filtered_adata_cond, color="score")
