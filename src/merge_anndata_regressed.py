...
cell_cycle_genes = [x for x in cell_cycle_genes if x in adata_cond.var_names.tolist()]

mito_genes = adata_cond.var_names[adata_cond.var_names.str.startswith('MT')].tolist()

nuisance_genes = cell_cycle_genes + mito_genes
...
vae.save("my_model_regressed/", overwrite = True)

vae = scvi.model.SCVI.load("my_model_regressed/", adata=adata_cond)
...
adata_cond.write_h5ad("output/scanpy/combined_adata_regressed.h5ad")

adata_cond = sc.read_h5ad("output/scanpy/combined_adata_regressed.h5ad")
