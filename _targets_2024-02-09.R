...
tar_plan(
  ...
  tar_target(
    figures_and_tables,
    copy_paper_data(clustree_compilation, 
    								collage_compilation, 
  ...
  tar_target(large_clone_comparisons,
    ...
    "SRR27187902" = list("2_v_1_1q+_2p+" = c("1k", "2b"), "4_v_2_16q-" = c("16a"))
  )),
  ...
  tar_target(study_qc_plots, plot_study_metadata(study_cell_stats)),
  ...
  tar_target(
    ...
    unlist(list(
      ...
      "interesting" = c("SETD6", "RBL2", "DDX19A", "EZH2", "RB1", "TFF1", "MDM4", "CD24", "ESRRG"),
  ...
  tar_target(subtype_violins,
    score_and_vlnplot_seu(final_seus, subtype_markers, leiden_cluster_file = "results/adata_filtered_metadata_0.25.csv"),
  ...
  # tar_target(patchwork_zphase_plot,
  #            plot_zphase_distribution_of_all_samples_by_scna(final_seus)
  #            ),

  tar_target(patchwork_phase_plot,
             plot_phase_distribution_of_all_samples_by_scna(final_seus)
  ),

# tar_target(test_subtype_violins,
#            score_and_vlnplot_seu("output/seurat/SRR14800536_filtered_seu.rds", subtype_markers, leiden_cluster_file = "results/adata_filtered_metadata_0.25.csv")
# ),
  ...
  tar_target(
    ...
    plot_putative_marker_across_samples(interesting_genes, unlist(regressed_seus), plot_type = VlnPlot, group_by = "clone_opt", cluster_dictionary, extension = "filtered")
  ),

  tar_target(
    marker_gene_vlnplots_by_cluster,
    plot_putative_marker_across_samples(interesting_genes, unlist(regressed_seus), plot_type = VlnPlot, group_by = "SCT_snn_res.0.4", cluster_dictionary, extension = "regressed")
  ),
...
tarchetypes::tar_file(cluster_file,
                      "data/raw_cluster_ids.txt"),

tarchetypes::tar_file(branch_dictionary_file,
					  "data/branch_dictionary.csv"),

tar_target(branch_dictionary, 
...
# asdf
  ...
  tar_target(filtered_recurrent_genes,
...
tar_target(filtered_recurrent_heatmap_file,
  ...
  tar_target(regressed_recurrent_genes,
...
tar_target(filtered_large_scna_prob_file, qpdf::pdf_combine(map_chr(filtered_numbat_heatmaps, 2), "results/filtered_scna_probabilities.pdf")),
  ...
  tar_target(all_diffex_clones,
    find_diffex_clones(final_seus, numbat_rds_files, large_clone_comparisons, cluster_dictionary, location = "all"),
    pattern = map(final_seus, numbat_rds_files, large_clone_comparisons, cluster_dictionary),
    iteration = "list"
  ),

  tar_target(all_diffex_clones_for_each_cluster,
    find_diffex_bw_clones_for_each_cluster(final_seus, numbat_rds_files,  large_clone_comparisons, cluster_orders, location = "all"),
    pattern = map(final_seus, numbat_rds_files, large_clone_comparisons, cluster_orders),
    iteration = "list"
  ),
  ...
  tar_target(table_all_diffex_clones,
  ...
  tar_target(cis_diffex_clones,
    find_diffex_clones(final_seus, numbat_rds_files, large_clone_comparisons, cluster_dictionary, location = "in_segment"),
    pattern = map(final_seus, numbat_rds_files, large_clone_comparisons, cluster_dictionary),
    iteration = "list"
  ),

  tar_target(cis_diffex_clones_for_each_cluster,
    find_diffex_bw_clones_for_each_cluster(final_seus, numbat_rds_files,  large_clone_comparisons, cluster_orders, location = "in_segment"),
    pattern = map(final_seus, numbat_rds_files, large_clone_comparisons, cluster_orders),
    iteration = "list"
  ),
  ...
  tar_target(table_cis_diffex_clones,
    tabulate_diffex_clones(
      cis_diffex_clones_for_each_cluster,
  ...
  tar_target(trans_diffex_clones,
    find_diffex_clones(final_seus, numbat_rds_files, large_clone_comparisons, cluster_dictionary, location = "out_of_segment"),
    pattern = map(final_seus, numbat_rds_files, large_clone_comparisons, cluster_dictionary),
    iteration = "list"
  ),

  tar_target(trans_diffex_clones_for_each_cluster,
    find_diffex_bw_clones_for_each_cluster(final_seus, numbat_rds_files,  large_clone_comparisons, cluster_orders, location = "out_of_segment"),
    pattern = map(final_seus, numbat_rds_files, large_clone_comparisons, cluster_orders),
    iteration = "list"
  ),


  tar_target(volcano_trans,
    make_volcano_diffex_clones(
  ...
  # kooi candidates
  tar_target(found_kooi_candidates,
  ...
  tar_target(table_trans_diffex_clones,
    tabulate_diffex_clones(
      trans_diffex_clones_for_each_cluster,
  ...
  tar_target(large_numbat_expression,
    retrieve_numbat_plot_type(large_numbat_pdfs, "exp_roll_clust.pdf")
  ),

tar_target(large_numbat_expression_file,
  ...
  tar_target(paired_clone_distribution_plots,
             calculate_clone_distribution(final_seus, cluster_orders, pairwise = TRUE),
...
tar_target(unpaired_clone_distribution_plots,
  ...
  tar_target(cluster_scorecard,
	...
	tar_target(debranched_clone_trees,
			   plot_clone_tree_from_path(debranched_seus, numbat_rds_files, large_clone_simplifications, legend = FALSE, horizontal = FALSE),
			   pattern = map(debranched_seus),
			   iteration = "list"
	),
  ...
  tar_target(annotated_heatmap_collages,
             plot_seu_marker_heatmap(final_seus, cluster_orders, numbat_rds_files, large_clone_simplifications),
             pattern = map(final_seus, cluster_orders, numbat_rds_files, large_clone_simplifications),
             iteration = "list"
  ),

	tar_target(debranched_annotated_heatmap_collages,
						 plot_seu_marker_heatmap(debranched_seus, cluster_orders, numbat_rds_files, large_clone_simplifications),
						 pattern = map(debranched_seus, cluster_orders, numbat_rds_files, large_clone_simplifications),
						 iteration = "list"
	),
...
tar_target(filtering_timelines,
  ...
  tar_target(oncoprint_input_by_scna_unfiltered,
    make_oncoprint_diffex_unfiltered(large_filter_expressions, cluster_dictionary, interesting_samples, cis_diffex_clones, trans_diffex_clones, large_clone_comparisons, n_slice = 20)
  ),
  ...
  tar_target(num_diffex_clone_scna_tally,
  ...
  tar_target(oncoprint_input_by_scna,
    make_oncoprint_diffex(large_filter_expressions, cluster_dictionary, interesting_samples, cis_diffex_clones, trans_diffex_clones, all_diffex_clones, large_clone_comparisons, n_slice = 20)
  ),

  tar_target(
    oncoprint_input_by_scna_for_each_cluster,
    make_oncoprint_diffex(large_filter_expressions, cluster_dictionary, interesting_samples, cis_diffex_clones_for_each_cluster, trans_diffex_clones_for_each_cluster, all_diffex_clones_for_each_cluster, large_clone_comparisons, n_slice = 20)
  ),

  tar_target(oncoprint_enrich_clones_gobp,
  ...
  tar_target(oncoprint_enrich_clones_hallmark,
    enrich_oncoprints(large_filter_expressions,
      cluster_dictionary,
      interesting_samples,
      cis_diffex_clones,
  ...
  tar_target(oncoprint_enrich_clones_plots_gobp,
  ...
  tar_target(oncoprint_enrich_clones_plots_hallmark,
...
tar_target(liu_lu_supp_data,
  ...
  tar_target(stemness_markers,
    pull_stem_cell_markers()
  ),

  # clusters enrichment ------------------------------
  # enrichment by cluster msigdb gobp
  tar_target(oncoprint_enrich_clusters_gobp,
  ...
  tar_target(oncoprint_enrich_clusters_hallmark,
    enrich_oncoprints_clusters(large_filter_expressions,
      cluster_dictionary,
      interesting_samples,
      cis_diffex_clones_for_each_cluster,
  ...
  tar_target(oncoprint_enrich_clusters_plots_gobp,
  ...
  tar_target(oncoprint_enrich_clusters_plots_hallmark,
