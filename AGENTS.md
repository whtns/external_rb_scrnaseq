...
A Slack incoming webhook is used to notify when long SLURM jobs finish.

- The webhook URL is stored at `~/.slack_webhook` (chmod 600, in `$HOME`, **not** in any repo).
- **NEVER print, echo, or paste the URL into a committed file or chat.** Always
  reference it indirectly as `"$(cat ~/.slack_webhook)"`.
- Send a message (Slack returns `ok` / HTTP 200 on success):
  ```bash
  curl -s -X POST -H 'Content-type: application/json' \
    --data '{"text":"my message"}' "$(cat ~/.slack_webhook)"
  ```
- **Preferred for job-finish pings:** a dependent notifier job that fires on ANY
  exit state of the target job (catches crash/timeout/OOM, unlike an inline curl
  at the end of a script, which only runs if the job reaches that line):
  ```bash
  sbatch --dependency=afterany:<JID> --job-name=slack_notify \
    ...
    --wrap='ST=$(sacct -j <JID> -n -o State | head -1 | xargs); \
            curl -s -X POST -H "Content-type: application/json" \
            --data "{\"text\":\"job <JID> finished: $ST\"}" "$(cat ~/.slack_webhook)"'
  ```
- Compute nodes on this allocation (epyc-64, verified) have outbound HTTPS, so the
  curl works from a compute-node job. If you ever hit a node without internet, run
  the notifier from the login/d05-29 node instead.
