...
enrichment_analysis <- function(df, fold_change_col = "avg_log2FC", analysis_method = "gsea", gene_set = "gobp", pvalueCutoff = 0.5){
  # browser()
  ...
  if(analysis_method == "gsea"){
    ...
    # gse@result[,"clone_nums"] <- unique(df$clone_nums)
    # gse@result[,"clone_comparison"] <- unique(df$clone_comparison)
...
plot_enrichment <- function(gse, p_val_cutoff = 0.1){

  make_signed_dotplot <- function(gse, showCategory=10){
    clusterProfiler::dotplot(gse, showCategory=showCategory, split=".sign", font.size = 12) + facet_grid(.sign~.)
  }

  possible_dotplot <- purrr::possibly(make_signed_dotplot, otherwise = ggplot())

  showCategories <- gse@compareClusterResult |> 
  	dplyr::filter(p.adjust <= 0.1) |> 
  	pull(ID)
  
  signed_dotplot <- possible_dotplot(gse, showCategory = showCategories)


  return(signed_dotplot)
}
...
enrich_by_cluster  <- function(numbat_rds_file){
  # browser()
...
make_faded_umap_plots <- function(full_seu, retained_clones){
	...
	cluster_plot <- DimPlot(full_seu, group.by = c("clusters")) + 
		aes(alpha = alpha_var) + 
		NULL
...
plot_seu_marker_heatmap_by_scna <- function(seu_path = NULL, cluster_order = NULL, nb_paths = NULL, clone_simplifications = NULL, group.by = "SCT_snn_res.0.6", assay = "SCT", label = "_filtered_", height = 10, width = 18, equalize_scna_clones = FALSE, phase_levels = c("g1", "g1_s", "s", "s_g2", "g2", "g2_m", "pm", "hsp", "hypoxia", "other", "s_star"), kept_phases = NULL, rb_scna_samples, large_clone_comparisons, scna_of_interest = "1q", min_cells_per_cluster = 50) {
	...
	if(!is.null(cluster_order)){
		...
		cluster_order <-
			cluster_order %>%
			dplyr::filter(preferred_resolution == "1") |> 
			dplyr::mutate(order = dplyr::row_number()) %>%
			dplyr::filter(!is.na(clusters)) %>%
			dplyr::mutate(clusters = as.character(clusters))
	...
	clone_distribution_plot <- plot_distribution_of_clones_across_clusters(
		seu, seu_name = glue("{tumor_id} {comparison_scna}"), var_x = "scna", var_y = "clusters", avg_line = clone_ratio, signif = TRUE, plot_type = "clone"
	)
	...
	umap_plots <- make_faded_umap_plots(full_seu, retained_clones)
	...
	ggsave(glue("results/{file_slug}_{scna_of_interest}_heatmap_phase_scatter_patchwork.pdf"), height = height, width = width)
